...
export default function PersonHoverCard({ name, role, currentMovieId }: PersonHoverCardProps) {
    ...
    return (
        ...
        >
            <span className="font-medium text-slate-800 dark:text-slate-200 cursor-pointer hover:text-blue-500 hover:underline decoration-blue-500/30 underline-offset-4 transition-colors">
                {name}
            </span>

            <AnimatePresence>
                {isHovered && related.length > 0 && (
                    <motion.div
                        initial={{ opacity: 0, y: 10, scale: 0.95 }}
                        animate={{ opacity: 1, y: 0, scale: 1 }}
                        exit={{ opacity: 0, y: 5, scale: 0.95 }}
                        transition={{ duration: 0.2 }}
                        className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-64 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-100 dark:border-slate-700 p-3 z-50"
                    >
